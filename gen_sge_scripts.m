function gen_sge_scripts(script_name, pattern, total_segments, num_job)
	
	script_dir = '/net/per610a/export/das11f/plsang/codes/lsvrc2010-v1';
	
	sge_sh_file = sprintf('%s/%s.sh', script_dir, script_name);
	
	
	[file_dir, file_name] = fileparts(sge_sh_file);
	output_dir = fullfile(script_dir, script_name);
